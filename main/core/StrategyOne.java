...
public class StrategyOne implements Play {
	...
	public int play(Game game) {
		...
		return 0;
		/*
		 * //&& game.playedFirst() int x = 0; System.out.println(hand.getSize()); //x =
		 * hand.IdRuns() + hand.IdSets(); //return x;
		 * 
		 * if(hand.getSize() > 0 && hand.IdSets() + hand.IdRuns() > 29) {
		 * System.out.print((int)hand.getSize()); //System.out.println(hand.IdSets() +
		 * hand.IdRuns());
		 * 
		 * 
		 * 
		 ...
		 * 
		 * if(hand.getSize() > 2) { System.out.println("!!!"); hand.sortHand();
		 * game.getPlayer().getHand().makeSet(game,game.getPlayer().getHand().get(0)); }
		 * }
		 * 
		 * 
		 * if(hand.getSize() == 0) { System.out.println(player.getName() +
		 * " WINS THE GAME!"); game.end(); return 1; } return 0; /
		 **/
	}
	...
	public void makeAdditions(Game game, Meld m, Tile t, int STOPT, int STOPM, int MELD, int TILE) {
		...
		if (STOPT == TILE && STOPM != MELD) {
			...
			makeAdditions(game, game.getTable().getMeld(MELD + 1), game.getPlayer().getTile(0), STOPM, STOPT, 0,
					MELD + 1);
		...
		if (currM.getTile(0).getValue() == currT.getValue() + 1) {
			...
			// call the function
			makeAdditions(game, game.getTable().getMeld(MELD), game.getPlayer().getTile(TILE + 1), STOPM, STOPT,
					TILE + 1, MELD);
